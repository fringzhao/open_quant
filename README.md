...
你可能会问：**现在 Tushare 有 MCP、Skill 等工具，AI Agent 都能直接下载数据了，为什么还需要这个仓库？**

确实，借助 Tushare MCP，你可以让 AI 直接获取数据。**但数据下载只是量化研究的第一步**——拿到数据后，你还需要：

- **数据存储与管理**：如何高效地存储、更新、查询海量行情数据？
- **因子计算**：如何基于原始行情计算各类定价因子？如何避免未来数据泄漏？
- **因子评估**：如何用科学的方法评估因子的 IC、IR、Sharpe？如何做分组回测？
- **交易模拟**：如何在回测中加入手续费、滑点、板块过滤，更贴近实盘？

这个仓库提供的就是**从数据到研究的完整工具链**——不只是帮你"拿到数据"，而是帮你**规范地做研究**，少踩坑、少走弯路。
...
```bash
# 推荐：同时获取示例因子
git clone --recursive https://github.com/ShenzhenLime/open_quant.git

# 已克隆但尚未初始化子模块
git submodule update --init --recursive

# 不需要示例因子
git clone https://github.com/ShenzhenLime/open_quant.git
```
